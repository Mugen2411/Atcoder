...
void AtcoderSolveHelper::Solve()
{
    ...
    Grid2D<int64_t> A(H, W, -1);
    Grid2D<int64_t> B(H, W, -1);

    auto _GetRowSwapped = [](const Grid2D<int64_t> &src, int64_t x) {
        int64_t nextX = x + 1;

        Grid2D<int64_t> retval(src);
        for (int64_t y = 0; y < retval.GetHeight(); ++y)
        {
            std::swap(retval.Ref(x, y), retval.Ref(nextX, y));
        }
        return retval;
    };

    auto _GetColumnSwapped = [](const Grid2D<int64_t> &src, int64_t y) {
        int64_t nextY = y + 1;

        Grid2D<int64_t> retval(src);
        for (int64_t x = 0; x < retval.GetWidth(); ++x)
        {
            std::swap(retval.Ref(x, y), retval.Ref(x, nextY));
        }
    ...
    std::unordered_map<Grid2D<int64_t>, int64_t> cur;
    cur[A] = 0;

    while (1)
    {
        std::unordered_map<Grid2D<int64_t>, int64_t> next;

        for (auto &c : cur)
        {
            for (int64_t x = 0; x < W - 1; ++x)
            {
                auto modded = _GetRowSwapped(c.first, x);
                if (next.count(modded))
                {
                    next[modded] = std::min(next[modded], c.second + 1);
                }
                else
                {
                    next[modded] = c.second + 1;
                }
            ...
            {
                auto modded = _GetColumnSwapped(c.first, y);
                if (next.count(modded))
                {
                    next[modded] = std::min(next[modded], c.second + 1);
                }
        ...
        int64_t prevSize = cur.size();

        for (auto &n : next)
        {
            if (cur.count(n.first))
            {
                cur[n.first] = std::min(cur[n.first], n.second);
            }
            else
            {
                cur[n.first] = n.second;
            }
        }

        int64_t curSize = cur.size();
