...
class AtcoderSolveHelper
{
  private:
    ...
    void Solve()
    {
        ...
        Grid2D<int64_t> pres(N + 1, N + 1, 0);
        for (int64_t y = 0; y < N; ++y)
        {
            for (int64_t x = 0; x < N; ++x)
            {
                pres.Ref(x + 1, y + 1) = (P[y][x] == 'B');
            }
        }
        ...
        for (int64_t x = 1; x <= N; ++x)
        {
            for (int64_t y = 1; y <= N; ++y)
            {
                pres.Ref(x, y) += pres.Ref(x, y - 1);
            }
        }

        auto _GetSum = [&](int64_t x1, int64_t y1, int64_t x2, int64_t y2) {
            int64_t retval = pres.Ref(x1, y1);
            retval += pres.Ref(x2, y2);
            retval -= pres.Ref(x1, y2);
            retval -= pres.Ref(x2, y1);
            return retval;
        };

        auto _GetSolid = [&](int64_t h, int64_t w) {
            int64_t numCompX = w / N;
            int64_t numCompY = h / N;

            int64_t retval = numCompX * numCompY * _GetSum(0, 0, N, N);
            retval += numCompY * _GetSum(0, 0, w % N, N);
            retval += numCompX * _GetSum(0, 0, N, h % N);
            retval += _GetSum(0, 0, w % N, h % N);
            return retval;
        };
