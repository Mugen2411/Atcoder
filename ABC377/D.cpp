...
void AtcoderSolveHelper::Solve()
{
    ...
    std::sort(ranges.begin(), ranges.end(), [](const RANGE &lhs, const RANGE &rhs) { return lhs.Left > rhs.Left; });

    int64_t curRight = -1;
    for (auto itr = ranges.begin(); itr != ranges.end(); ++itr)
    {
        if (curRight == -1 || curRight > itr->Right)
        {
            curRight = itr->Right;
            continue;
        }
    ...
    {
        auto itr = std::lower_bound(ranges.begin(), ranges.end(), RANGE{.Left = m},
                                    [&m](const RANGE &lhs, const RANGE &rhs) { return lhs.Left < rhs.Left; });
        if (itr == ranges.end())
        {
            ans += (M - m + 1);
            //Error() << (M - m + 1) << std::endl;
        }
        else
        {
            ans += (itr->Right - m);
            //Error() << (itr->Right - m) << std::endl;
        }
    }
    Out() << ans;
}
