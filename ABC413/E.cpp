...
private:
    ...
    void Solve()
    {
        int64_t N;
        In() >> N;
        std::vector<int64_t> P(1 << N);
        EachInput(P);

        auto rec = [&](auto self, int64_t lIdx, int64_t depth)
        {
            if (depth == N)
            {
                return;
            }
            int64_t size = 1 << (N - depth);
            int64_t halfSize = size >> 1;

            self(self, lIdx, depth + 1);
            self(self, lIdx + halfSize, depth + 1);

            bool needReverse = false;
            for (int64_t i = 0; i < 1; ++i)
            {
                if (P[lIdx + i] > P[lIdx + halfSize + i])
                {
                    needReverse = true;
                    break;
                }
            ...
            if (needReverse)
            {
                ...
                {
                    std::swap(P[lIdx + i], P[lIdx + halfSize + i]);
                }
            }
        };
