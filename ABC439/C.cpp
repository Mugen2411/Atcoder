...
int64_t FloorSqrt(int64_t n)
{
    int64_t ans = std::sqrt(n);
    while (ans * ans > n)
    {
        --ans;
    }
...
void AtcoderSolveHelper::Solve()
{
    ...
    int64_t sqN = FloorSqrt(N);
    std::vector<int64_t> n2;
    for (int64_t i = 1; i <= sqN; ++i)
    {
        n2.push_back(i * i);
    }
    std::unordered_map<int64_t, int64_t> already;
    std::vector<int64_t> ans;
    for (int64_t i = 0; i < n2.size() - 1; ++i)
    {
        for (int64_t j = i + 1; j < n2.size(); ++j)
        {
            int64_t tmp = n2[i] + n2[j];
            if (tmp > N)
            {
                continue;
            }
