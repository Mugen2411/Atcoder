...
void AtcoderSolveHelper::Solve()
{
    ...
    std::map<int64_t, int64_t> nega;
    std::map<int64_t, int64_t> posi;
    ...
    for (int64_t left = 0; left < N; ++left)
    {
        ...
        {
            int64_t cur = A[right];
            auto itr1 = nega.lower_bound(-cur);
            int64_t dist1 = (itr1 == nega.end()) ? D : std::abs(-cur - itr1->first);
            auto itr2 = posi.lower_bound(cur);
            int64_t dist2 = (itr2 == posi.end()) ? D : std::abs(cur - itr2->first);
            if (dist1 < D || dist2 < D)
            {
                break;
            }
        ...
        {
            int64_t aft1 = --nega[-A[left]];
            if (aft1 == 0)
            {
                nega.erase(-A[left]);
            }
            int64_t aft2 = --posi[A[left]];
            if (aft2 == 0)
            {
                posi.erase(A[left]);
            }
