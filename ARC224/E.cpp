...
void AtcoderSolveHelper::Solve()
{
    ...
    for (auto c : S)
    {
        ...
        if (st.size() >= 3 && st[st.size() - 3] == 'C' && st[st.size() - 2] == 'B' && st[st.size() - 1] == 'A')
        {
            st.pop_back();
            st.pop_back();
            st.pop_back();
        }
        else if (st.size() >= 2 && st[st.size() - 2] == 'B' && st[st.size() - 1] == 'A')
        {
            st.pop_back();
            st.pop_back();
        }
        else if (st.size() >= 1 && st[st.size() - 1] == 'A')
        {
            st.pop_back();
        }
    }

    Out() << st.size() << std::endl;
}
