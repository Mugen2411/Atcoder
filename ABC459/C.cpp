...
void AtcoderSolveHelper::Solve()
{
    int64_t N, Q;
    In() >> N >> Q;
    std::vector<int64_t> field(N, 0);
    std::unordered_map<int64_t, std::unordered_set<int64_t>> num;
    for (int i = 0; i < N; ++i)
    {
        num[0].insert(i);
    }
    ...
    while (Q--)
    {
        ...
        case 1: {
            ...
            int64_t prev = field[x];
            int64_t next = field[x] + 1;
            ++field[x];
            num[next].insert(x);
        ...
        case 2: {
            ...
            Out() << num[y + removed].size() << std::endl;
        }
