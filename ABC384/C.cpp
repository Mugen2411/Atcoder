...
void AtcoderSolveHelper::Solve()
{
    ...
    for (auto b = BitManager::AllFalse(); b != BitManager::Onehot(5); ++b)
    {
        std::string name;
        int64_t score = 0;
        for (int i = 0; i < 5; ++i)
        ...
        person.push_back(Person{.Name = name, .score = score});
    }

    std::sort(person.begin(), person.end(), [](const Person &lhs, const Person &rhs) {
