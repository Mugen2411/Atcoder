...
void AtcoderSolveHelper::Solve()
{
    ...
    const int64_t dir[][2] = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};

    auto _Dfs1 = [&](auto self, int64_t curIdx) {
        ...
        for (auto &d : dir)
        {
            int64_t nextX = curX + d[0];
            int64_t nextY = curY + d[1];

            if (S.IsInner(nextX, nextY))
            {
                self(self, S.GetIndex(nextX, nextY));
            }
        }
    };

    int64_t ans = 0;
    auto _Dfs2 = [&](auto self, int64_t curIdx, bool isFirst) -> void {
        int64_t curX = S.GetX(curIdx);
        int64_t curY = S.GetY(curIdx);
        ...
        for (auto &d : dir)
        {
            ...
            if (S.IsInner(nextX, nextY))
            {
                self(self, S.GetIndex(nextX, nextY), false);
            }
        }
    };
    ...
    for (int x = 0; x < W; ++x)
    {
        _Dfs1(_Dfs1, S.GetIndex(x, 0));
        _Dfs1(_Dfs1, S.GetIndex(x, H - 1));
    }
    for (int y = 0; y < H; ++y)
    {
        _Dfs1(_Dfs1, S.GetIndex(0, y));
        _Dfs1(_Dfs1, S.GetIndex(W - 1, y));
    }
    for (int y = 0; y < H; ++y)
    {
        for (int x = 0; x < W; ++x)
        {
            _Dfs2(_Dfs2, S.GetIndex(x, y), true);
        }
    }
    Out() << ans;
}
