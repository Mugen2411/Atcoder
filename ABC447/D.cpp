...
void AtcoderSolveHelper::Solve()
{
    ...
    std::set<int64_t> idxA, idxB, idxC;
    idxA.insert(99);
    idxB.insert(99);
    idxC.insert(99);
    int64_t ans = 0;
    for (int i = 0; i < S.size(); ++i)
    {
        ...
        while (!idxC.empty())
        {
            auto itrC = idxC.begin();
            if (*itrC == 99)
            {
                break;
            }
            auto itrB = idxB.lower_bound(*itrC);
            if (itrB == idxB.end() || *itrB == 99)
            {
                break;
            }
            auto itrA = idxA.lower_bound(*itrB);
            if (itrA == idxA.end() || *itrA == 99)
            {
                break;
            }
            ++ans;
