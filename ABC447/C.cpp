...
void AtcoderSolveHelper::Solve()
{
    ...
    std::string S, T;
    std::vector<COMP> compS, compT;
    compS.push_back(COMP{.Val = ' ', .NumA = 0});
    compT.push_back(COMP{.Val = ' ', .NumA = 0});
    In() >> S >> T;
    for (auto c : S)
    {
        if (c == 'A')
        ...
        {
            compS.push_back(COMP{.Val = c, .NumA = 0});
        }
    }
    for (auto c : T)
    {
        ...
        {
            compT.push_back(COMP{.Val = c, .NumA = 0});
        }
    }
    if (compS.size() != compT.size())
    {
        Out() << -1;
        return;
    }
    ...
    {
        if (compS[i].Val != compT[i].Val)
        {
            Out() << -1;
            return;
        }
        ans += std::abs(compS[i].NumA - compT[i].NumA);
    }
    Out() << ans;
}
