...
void AtcoderSolveHelper::Solve()
{
    ...
    const int64_t MAX_DOUBLING = 30;
    std::vector<std::vector<int64_t>> route(N);
    std::vector<std::vector<int64_t>> volume(N);
    for (auto &i : route)
    {
        i.resize(MAX_DOUBLING);
    }
    for (auto &i : volume)
    {
        i.resize(MAX_DOUBLING);
    ...
    for (int64_t d = 1; d < MAX_DOUBLING; ++d)
    {
        for (int64_t i = 0; i < N; ++i)
        {
            int64_t midIdx = route[i][d - 1] - 1;
            route[i][d] = route[midIdx][d - 1];
            volume[i][d] = volume[i][d - 1] + volume[midIdx][d - 1];
        }
    }
    while (Q--)
    {
        ...
        while (T > 0)
        {
            if (T & 1)
            {
                vol += volume[idx - 1][base];
                idx = route[idx - 1][base];
            }
            ++base;
            T >>= 1;
        }
