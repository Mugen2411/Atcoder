...
void AtcoderSolveHelper::Solve()
{
    ...
    for (auto &&r : R)
    {
        In() >> r.Begin >> r.Duration;
        r.End = r.Begin + r.Duration;
    }

    std::sort(R.begin(), R.end(), [](const RANGE &lhs, const RANGE &rhs) {
    ...
    std::priority_queue<RANGE> curRange;
    int64_t ans = 0;
    for (int64_t time = 0;; ++time)
    {
        if (curRange.empty())
        {
            if (R.empty())
        ...
        while (!R.empty() && R.back().Begin == time)
        {
            curRange.push(R.back());
            R.pop_back();
        }
        ...
        while (!curRange.empty())
        {
            auto &top = curRange.top();
            if (top.End == time)
            {
                curRange.pop();
            }
        ...
        //Error() << "size: " << curRange.size() << " ans: " << ans << std::endl;
    }
    Out() << ans;
}
