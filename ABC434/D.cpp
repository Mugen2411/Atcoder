...
private:
    ...
    void Solve()
    {
        ...
        struct ELEM
        {
            std::unordered_set<int64_t> parent;
            int64_t covered;
        };

        const int64_t H = 2000;
        const int64_t W = 2000;
        Grid2D<int64_t> covered(H + 1, W + 1, 0);
        Grid2D<int64_t> parent(H + 1, W + 1, 0);

        for (int i = 1; i <= N; ++i)
        {
            int64_t U, D, L, R;
            In() >> U >> D >> L >> R;
            parent.Ref(L - 1, U - 1) += i;
            parent.Ref(L - 1, D) += -i;
            parent.Ref(R, U - 1) += -i;
            parent.Ref(R, D) += i;

            covered.Ref(L - 1, U - 1) += 1;
            covered.Ref(L - 1, D) += -1;
            covered.Ref(R, U - 1) += -1;
            covered.Ref(R, D) += 1;
        }
        ...
        for (int y = 0; y <= H; ++y)
        {
            for (int x = 1; x <= W; ++x)
            {
                parent.Ref(x, y) += parent.Ref(x - 1, y);
                covered.Ref(x, y) += covered.Ref(x - 1, y);
            }
        }
        ...
        for (int y = 1; y <= H; ++y)
        {
            for (int x = 0; x <= W; ++x)
            {
                parent.Ref(x, y) += parent.Ref(x, y - 1);
                covered.Ref(x, y) += covered.Ref(x, y - 1);
            }
        }

        int64_t all = 0;
        std::unordered_map<int64_t, int64_t> cost;
        for (int y = 0; y <= H; ++y)
        {
            for (int x = 0; x <= W; ++x)
            {
                if (covered.Ref(x, y) != 0)
                {
                    ++all;
                }
                if (covered.Ref(x, y) != 1)
                {
                    continue;
                }
