...
int main()
{
    ...
    SegmentTree<int64_t> segTree(N, std::numeric_limits<int64_t>::min(), [](int64_t lhs, int64_t rhs) -> int64_t
    ...
    while (Q--)
    {
        ...
        case 3:
        {
            ...
            std::cin >> X >> V;
            --X;
            size_t pos = segTree.FindLeft(X, N, [V](int64_t i)
                                          { return V > i; });
            if (pos == SegmentTree<int64_t>::INVALID)
            {
