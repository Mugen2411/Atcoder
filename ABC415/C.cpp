...
private:
    ...
    void Solve()
    {
        ...
        std::unordered_set<BitManager> mask;
        ...
        std::unordered_set<BitManager> searched;
        bool hasAnswer = false;
        auto dfs = [&](auto self, BitManager cur) -> void
        {
            if (hasAnswer)
            {
                return;
            }
            if (cur == BitManager::Filled(N))
            {
                hasAnswer = true;
                return;
            }
            ...
            for (int i = 0; i < N; ++i)
            {
                if (cur.Get(i) == 0)
                {
                    BitManager next = cur;
                    next.Set(i, true);
                    if (mask.count(next) == 0 && searched.count(next) == 0)
                    {
