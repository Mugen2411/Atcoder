...
int main()
{
    ...
    while (!q.empty())
    {
        int64_t curIdx = q.front();
        q.pop();
        int64_t curX = S.GetX(curIdx);
        int64_t curY = S.GetY(curIdx);
        if (S.IsInner(curX, curY - 1) && S.Ref(curX, curY - 1) == '.')
        {
            S.Ref(curX, curY - 1) = 'v';
            q.push(S.GetIndex(curX, curY - 1));
        }
        if (S.IsInner(curX, curY + 1) && S.Ref(curX, curY + 1) == '.')
        {
            S.Ref(curX, curY + 1) = '^';
        ...
        if (S.IsInner(curX - 1, curY) && S.Ref(curX - 1, curY) == '.')
        {
            S.Ref(curX - 1, curY) = '>';
            q.push(S.GetIndex(curX - 1, curY));
        }
        if (S.IsInner(curX + 1, curY) && S.Ref(curX + 1, curY) == '.')
        {
            S.Ref(curX + 1, curY) = '<';
