...
private:
    ...
    void Solve()
    {
        ...
        while (!idxQ.empty())
        {
            ...
            int64_t curX = A.GetX(curIdx);
            int64_t curY = A.GetY(curIdx);

            if (A.Ref(curX, curY) == 'G')
            {
            ...
            for (auto &dir : RLUD)
            {
                ...
                if (A.IsInner(nextX, nextY))
                {
                    int64_t nextIdx = A.GetIndex(nextX, nextY);

                    if (isSwitched)
                    {
                        if (!searched2[nextIdx] && A.Ref(nextX, nextY) != '#' && A.Ref(nextX, nextY) != 'o')
                        {
                            idxQ.push(nextIdx);
                    ...
                    {
                        if (!searched1[nextIdx] && A.Ref(nextX, nextY) != '#' && A.Ref(nextX, nextY) != 'x')
                        {
                            idxQ.push(nextIdx);
                            isSwitchedQ.push(isSwitched);
                            distanceQ.push(distance + 1);
