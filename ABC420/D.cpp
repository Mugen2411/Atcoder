...
void AtcoderSolveHelper::Solve()
{
    ...
    std::vector<bool> searched1(H * W);
    std::vector<bool> searched2(H * W);

    std::string RLDU("RLDU");

    const int64_t INVALID = 0xFFFFFFFFFFFFFF;
    int64_t ans = INVALID;

    std::queue<Grid2D<char>::Iterator> itrQ;
    std::queue<bool> isSwitchedQ;
    std::queue<int64_t> distanceQ;

    itrQ.push(A.GetItr(startIdx));
    isSwitchedQ.push(false);
    distanceQ.push(0);
    ...
    while (!itrQ.empty())
    {
        ...
        itrQ.pop();
        bool isSwitched = isSwitchedQ.front();
        isSwitchedQ.pop();
        int64_t distance = distanceQ.front();
        distanceQ.pop();
        ...
        if (!isSwitched)
        {
            if (searched1[curItr.GetIndex()])
            {
                continue;
            }
            searched1[curItr.GetIndex()] = true;
        }
        else
        {
            if (searched2[curItr.GetIndex()])
            {
                continue;
            }
            searched2[curItr.GetIndex()] = true;
        }

        if (*curItr == '?')
        {
            isSwitched = (isSwitched) ? 0 : 1;
        }
        for (auto &dir : RLDU)
        {
            POSITION next = POSITION::RLDU(dir);
            auto nextItr = curItr + next;

            if (nextItr.IsInner())
            {
                if (isSwitched)
                {
                    if (!searched2[nextItr.GetIndex()] && *nextItr != '#' && *nextItr != 'o')
                    {
                        itrQ.push(nextItr);
                        isSwitchedQ.push(isSwitched);
                        distanceQ.push(distance + 1);
                    }
                }
                else
                {
                    if (!searched1[nextItr.GetIndex()] && *nextItr != '#' && *nextItr != 'x')
                    {
                        itrQ.push(nextItr);
                        isSwitchedQ.push(isSwitched);
                        distanceQ.push(distance + 1);
