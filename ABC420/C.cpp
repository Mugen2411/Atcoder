...
private:
    ...
    void Solve()
    {
        ...
        {
            int64_t a = BinarySearch(l - 1, N + 1, [&](int64_t cur)
                                     { return numA[cur] - numA[l] >= A; });

            if (a == l - 1 || a == N + 1)
            {
                continue;
            }
            int64_t b = BinarySearch(N + 1, l - 1, [&](int64_t cur)
                                     { return numB[cur] - numB[l] < B; });

            if (b == N + 1 || b == l - 1)
            {
                continue;
            }
            ans += std::max<int64_t>(0ll, b - a + 1);
        }
