...
void AtcoderSolveHelper::Solve()
{
    ...
    std::unordered_map<int64_t, std::unordered_map<int64_t, int64_t>> command;
    ...
    for (int j = 1; j <= M; ++j)
    {
        if (command.count(j) != 0)
        {
            for (auto &c : command[j])
            {
                if (c.first < 0)
                {
                    current[-c.first] -= c.second;
                    if (current[-c.first] == 0)
                    {
                        current.erase(-c.first);
                    }
                }
                else
                {
