...
void AtcoderSolveHelper::Solve()
{
    ...
    std::vector<int64_t> benefit(N - 1);
    ...
    if (N == 2)
    {
        Out() << std::max<int64_t>(0, benefit[0]) << std::endl;
        return;
    }

    std::vector<int64_t> dp(N, 0);
    dp[0] = 0;
    dp[1] = std::max<int64_t>(0, benefit[0]);
    dp[2] = std::max<int64_t>(0, benefit[1]);

    for (int i = 3; i < N; ++i)
    {
        if (benefit[i - 1] < 0)
        {
            dp[i] = std::max<int64_t>(dp[i - 3], dp[i - 2]);
        }
        else
        {
            dp[i] = std::max<int64_t>(dp[i - 3] + benefit[i - 1], dp[i - 2] + benefit[i - 1]);
        }
    }

    Out() << std::max<int64_t>(dp[N - 1], dp[N - 2]) << std::endl;
}
