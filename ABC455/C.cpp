...
void AtcoderSolveHelper::Solve()
{
    ...
    std::vector<int64_t> factor;
    for (auto p : mp)
    {
        factor.push_back(p.first * p.second);
    }
    std::sort(factor.begin(), factor.end(), std::greater<int64_t>());
    for (int i = 0; i < std::min<int64_t>(factor.size(), K); ++i)
    {
        sum -= factor[i];
    }
    Out() << sum;
