...
void AtcoderSolveHelper::Solve()
{
    ...
    std::vector<int64_t> searched(N, 0);
    std::vector<int64_t> ans(N, 0);

    auto _Dfs = [&](auto self, int64_t cur, int64_t cost) -> int64_t {
        if (searched[cur] < 0)
        {
            ans[-searched[cur] - 1] += cost;
            return searched[cur];
        }
        if (nodes[cur].Prev < 0)
        {
            ...
            ans[-nodes[cur].Prev - 1] += cost;
            searched[cur] = nodes[cur].Prev;
            return nodes[cur].Prev;
        }
        return searched[cur] = self(self, nodes[cur].Prev, cost + 1);
    };
