...
void AtcoderSolveHelper::Solve()
{
    ...
    std::vector<BURGER> B(N + 1);
    B[0] = BURGER{.size = 1, .patty = 1};
    for (int i = 1; i <= N; ++i)
    {
        B[i] = BURGER{.size = B[i - 1].size * 2 + 3, .patty = B[i - 1].patty * 2 + 1};
    }

    int64_t ans = 0;
    auto _Dfs = [&](auto self, int64_t up, int64_t down, int64_t cur, int64_t dimension) {
        if (dimension == 1)
        {
            ans += std::min<int64_t>(3, std::max<int64_t>(0, cur));
            return;
        }

        if (cur == up)
        {
            ans += B[dimension].patty;
            return;
        }
        ...
        int64_t mid = (up + down) / 2;
        if (cur == mid)
        {
            ans += B[dimension - 1].patty + 1;
            return;
        }
        if (cur > mid)
        {
            ans += B[dimension - 1].patty + 1;
            self(self, up - mid - 2, down, cur - mid - 1, dimension - 1);
        }
        else
        {
            self(self, up - mid - 2, down, cur - 1, dimension - 1);
        }
    };

    _Dfs(_Dfs, B[N].size - 1, 0, X - 1, N);
