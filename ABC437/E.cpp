...
class TrieTree
{
  ...
  public:
    ...
    void Dfs(Func f)
    {
        ...
        _Dfs(_Dfs, m_createdNodes[0].get());
    }
...
void AtcoderSolveHelper::Solve()
{
    ...
    TrieTree<int64_t> tt;
    std::vector<TrieTree<int64_t>::NODE *> parents;
    std::unordered_map<TrieTree<int64_t>::NODE *, std::set<int64_t>> indices;
    parents.push_back(tt.GetRootNode());
    ...
    for (int i = 1; i <= N; ++i)
    {
        ...
        parents.push_back(tt.Insert(y, parents[x]));
        indices[parents.back()].insert(i);
    }

    tt.Dfs([&](TrieTree<int64_t>::NODE *pNode) {
        for (auto &i : indices[pNode])
        {
            Out() << i << " ";
        }
    });
