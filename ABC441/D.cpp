...
void AtcoderSolveHelper::Solve()
{
    ...
    std::vector<bool> isAns(N, false);
    auto _Dfs = [&](auto self, int64_t cur, int64_t totalCost, int64_t numPassed) {
        if (numPassed == L)
        {
            if (S <= totalCost && totalCost <= T)
            {
                isAns[cur] = true;
            }
            return;
        ...
        {
            self(self, e.To, totalCost + e.Cost, numPassed + 1);
        }
    };
    _Dfs(_Dfs, 0, 0, 0);
    for (int i = 0; i < N; ++i)
