...
void AtcoderSolveHelper::Solve()
{
    ...
    for (int i = 0; i <= 33; ++i)
    {
        base.push_back(std::to_string(cur));
        cur <<= 1;
    }

    std::unordered_set<int64_t> all;

    auto _Dfs = [&](auto self, std::string curStr) -> void {
        if (!curStr.empty() && curStr.size() < 10)
        {
            all.insert(std::stoll(curStr));
        }
        for (auto &s : base)
        {
            if (curStr.size() + s.size() < 10)
            {
                self(self, curStr + s);
            }
        }
    ...
    _Dfs(_Dfs, std::string());

    std::vector<int64_t> ans(all.begin(), all.end());
