...
void AtcoderSolveHelper::Solve()
{
    ...
    std::unordered_map<int64_t, std::vector<int64_t>> inEvent;
    std::unordered_map<int64_t, std::vector<int64_t>> outEvent;
    int64_t ans = 0;
    ...
    for (int i = 1; i <= N; ++i)
    {
        ...
        if (T - S >= D)
        {
            inEvent[S - 1].push_back(i);
            outEvent[T - 1 - D].push_back(i);
        }
    }

    std::unordered_map<int64_t, int64_t> curEntry;

    for (int d = 0; d < 1000001; ++d)
    {
        if (inEvent.count(d) != 0)
        {
            for (auto i : inEvent[d])
            {
                ++curEntry[i];
            }
        }

        ans += curEntry.size() * (curEntry.size() - 1) / 2;

        if (outEvent.count(d) != 0)
        {
            for (auto o : outEvent[d])
            {
                curEntry.erase(o);
            }
        }
